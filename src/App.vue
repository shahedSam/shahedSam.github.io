  ...
  <div class="container">
    ...
    <main>
      ...
      <section class="buttonContainer">
        <button type="button" class="btn btn-warning" @click="handleCheckButtonClick"
          :disabled="answers.size !== questions.length || isCheckingMode">Проверить</button>
        <button type="button" class="btn btn-warning" @click="handleResetButtonClick">Сбросить</button>
      </section>
      <template v-if="isCheckingMode">
        ...
        <section class="feedback">
          <p v-if="feedbackHasSent">Если не помешал блокировщик рекламы, автор получит отзыв
          </p>
          <template v-else>
            <div>
            ...
            <button type="button" class="btn btn-warning" @click="hanldeSendFeedback"
...
const handleCheckButtonClick = async () => {
  ...
  const messageForSend = `[Шахед] Кто-то прошёл тест c результатом "${resultText.value}".`;
...
const hanldeSendFeedback = () =>
  fetch(urlBase + "[Шахед] Отзыв: " + feedbackText.value).then(() => {
    feedbackText.value = "";
